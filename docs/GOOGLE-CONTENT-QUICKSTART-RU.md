...
- `01 Site Translations` — весь интерфейс, страницы, SEO, aria-label и общие alt. EN — исходный язык; DE, PT и ES — переводы.
...
4. Введите перевод в DE, PT или ES. Для `rich` используйте Markdown, не HTML.
5. Не меняйте `Key`: это технический контракт с кодом.

Пустая locale-ячейка означает English fallback. Например, пустой DE для кнопки на `/de/games` покажет английский текст, но URL останется немецким.
...
1. В `01 Site Translations` временно заполните DE для `footer.company`, нажмите Publish и откройте `/de`.
2. Очистите тестовый DE: на `/de` должен появиться English fallback, без key/undefined.
3. В `03 Game Translations` измените DE `pirates-rush / overview`, опубликуйте и откройте `/de/portfolio/pirates-rush`.
