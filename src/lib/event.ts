...
export function eventDateRange(event: EventData, month: 'long' | 'short' = 'long') {
  const format = new Intl.DateTimeFormat('en-GB', { day: 'numeric', month, timeZone: event.timeZone });
...
export function meetingDays(event: EventData) {
  const days: { value: string; label: string }[] = [];
  const label = new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'long', timeZone: event.timeZone });
